...
void init_edges() {
    edge_map.clear();
    while (!edge_queue.empty()) edge_queue.pop();
...
bool contract_edge(const Edge &e) {
    int v1 = e.v1, v2 = e.v2;
    if (!vertex_neighbors[v1].count(v2)) return false;
...
void cleanup() {
    ...
    for (auto &f : faces) {
        f.x = remap[f.x]; f.y = remap[f.y]; f.z = remap[f.z];
    }
    vertices.swap(newV);
}
...
void simplify(float targetRatio) {
    ...
    while (faces.size() > targetFaces && !edge_queue.empty()) {
        ...
        if (!edge_map.count(k) || fabs(edge_map[k].cost - e.cost) > 1e-9) continue;
        if (contract_edge(e)) edge_map.erase(k);
    }
    cleanup();
}
